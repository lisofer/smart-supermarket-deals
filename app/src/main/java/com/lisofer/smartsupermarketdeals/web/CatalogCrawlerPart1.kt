...
(() => {
  if (window.__smartDealsCrawlerV14) return;
  window.__smartDealsCrawlerV14 = true;
  const KEY = '__smartDealsCrawlerV14State';
  const CRAWLER_TOTAL_MS = 900000;
  const ROUTE_SLICE_MS = 55000;
  ...
  const blocked = /(?:checkout|carrito|cart|login|registro|perfil|account|pedido|order|help|ayuda|terminos|privacidad)/i;
  ...
  const load = () => { try { const x=JSON.parse(localStorage.getItem(KEY)||'null'); if(x?.root) return x; } catch(_){} return fresh(location.href); };
  let state = load();
  ...
  window.__smartDealsResetCatalogCrawler = value => {
    root=abs(value||root||location.href);
    state=fresh(root);
    sent=new Set();
    save();
    event('catalog_reset',{root:state.root});
  };
  ...
  const parts = card => {
    ...
    Array.from(card.querySelectorAll('[aria-label],[title],[alt],[data-testid],[data-promotion],[data-discount],[class*="promo"],[class*="Promo"],[class*="discount"],[class*="Discount"],[class*="badge"],[class*="Badge"]')).slice(0,75).forEach(node=>{
  ...
  const priceOf = card => {
    const leaves=Array.from(card.querySelectorAll('*')).slice(0,140).filter(e=>e.children.length===0&&money.test(clean(e.innerText||e.textContent)));
  ...
  const promoOf = text => {
    ...
    if(second.test(text)&&p){const v=p[1].replace(',','.');return {label:'2DA UNIDAD '+v+'% OFF',commercial:{mechanic:{type:'SECOND_UNIT'},benefit:{type:'PERCENTAGE',value:v},rawText:text.slice(0,1600)}};}
    if(second.test(text)&&/(?:gratis|sin\s*cargo)/i.test(text))return {label:'2x1',commercial:{mechanic:{type:'SECOND_UNIT_FREE'},rawText:text.slice(0,1600)}};
    const m=multi.exec(text); if(m)return {label:m[1]+'x'+m[2],commercial:{rawText:text.slice(0,1600)}};
    const t=takePay.exec(text); if(t)return {label:'LLEVÁ '+t[1]+' PAGÁ '+t[2],commercial:{rawText:text.slice(0,1600)}};
    if(p&&promoWord.test(text))return {label:p[1].replace(',','.')+'% OFF',commercial:{type:'PERCENTAGE',value:p[1],rawText:text.slice(0,1600)}};
    return null;
  };
  const scan = async phase => {
    const found=[]; const cards=Array.from(document.querySelectorAll(selector)).slice(0,2500);
    for(let i=0;i<cards.length;i++){
      const card=cards[i], visible=clean(card.innerText||card.textContent); if(visible.length<5||visible.length>5000||!money.test(visible))continue;
      ...
      found.push({id:clean(id).slice(0,340),name,price:price.now,originalPrice:price.before,promotionLabel:promo?.label||null,promotionText:promo?.label||null,commercial:promo?.commercial||null,rawCommercialText:raw.slice(0,2200),source:'catalog-route-dom-v14'});
      if(i&&i%24===0)await idle();
    }
    for(let i=0;i<found.length;i+=60)post({url:location.href+'#route-v14-'+Date.now()+'-'+i,body:JSON.stringify({products:found.slice(i,i+60)})});
    state.step++; save(); event('explore_progress',{step:state.step,phase,pageProducts:found.length,pageCards:cards.length,pending:state.pending.length,visited:state.visited.length,route:location.href});
    return found.length;
  };
